...
import { loginRoute } from "../utilshttp://171.238.155.142:8080/apiRoutes";
...
export default function Login() {
  ...
  const validateForm = () => {
    ...
    if (username === "") {
      toast.error("Chưa nhập Email và Password.", toastOptions);
      return false;
    } else if (password === "") {
      toast.error("Chưa nhập Email và Password.", toastOptions);
      return false;
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      ...
      if (data.status === true) {
        ...
        localStorage.setItem("UserAvata", data.user.avata);
